describe("db.native", () => {
  ...
  it("execute uses getAllAsync", async () => {
    ...
    const native = require("../db.native");
    const rows = await native.execute("SELECT * FROM test", [1]);
  ...
  it("executeVoid uses runAsync", async () => {
    ...
    const native = require("../db.native");
    await native.executeVoid("UPDATE test SET a=?", [2]);
